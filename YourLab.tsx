...
import { SearchIcon, ChevronRightIcon, PlusIcon, DumbbellIcon, ClipboardListIcon, TrashIcon, TrophyIcon, BrainIcon, ActivityIcon, TargetIcon } from './icons';
...
const YourLab: React.FC = () => {
    ...
    const specialCategories = useMemo(() => {
        ...
        // Exclude 'Core' as it has its own special section now
        return allSpecial.filter(cat => cat !== 'Core');
    }, [muscleHierarchy]);
    ...
    const renderExploreTab = () => (
        <div className="space-y-6">
            {searchQuery.length > 0 ? (
                ...
                <>
                    {/* BodyLab Section */}
                    <div onClick={() => navigateTo('body-lab')} className="glass-card !p-4 border-2 border-sky-600/50 bg-gradient-to-br from-sky-900/20 to-transparent cursor-pointer flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <BrainIcon size={24} className="text-sky-300"/>
                            <div>
                                <h2 className="text-xl font-bold text-sky-300">BodyLab</h2>
                                <p className="text-xs text-slate-400">Descubre tu perfil de atleta único con IA.</p>
                            </div>
                        </div>
                        <ChevronRightIcon className="text-sky-400" />
                    </div>

                    {/* Training Purpose Section */}
                    <div onClick={() => navigateTo('training-purpose')} className="glass-card !p-4 border-2 border-purple-600/50 bg-gradient-to-br from-purple-900/20 to-transparent cursor-pointer flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <TargetIcon size={24} className="text-purple-300"/>
                            <div>
                                <h2 className="text-xl font-bold text-purple-300">Laboratorio de Propósitos</h2>
                                <p className="text-xs text-slate-400">Encuentra ejercicios para tus metas específicas.</p>
                            </div>
                        </div>
                        <ChevronRightIcon className="text-purple-400" />
                    </div>

                    {/* Mobility Lab Section */}
                    <div onClick={() => navigateTo('mobility-lab')} className="glass-card !p-4 border-2 border-green-600/50 bg-gradient-to-br from-green-900/20 to-transparent cursor-pointer flex justify-between items-center">
                        <div className="flex items-center gap-4">
                            <ActivityIcon size={24} className="text-green-300"/>
                            <div>
                                <h2 className="text-xl font-bold text-green-300">Laboratorio de Movilidad</h2>
                                <p className="text-xs text-slate-400">Genera rutinas de calentamiento personalizadas.</p>
                            </div>
                        </div>
                    ...
                    {/* CORE Section */}
                    <div onClick={() => navigateTo('chain-detail', { chainId: 'Core' })} className="glass-card !p-4 border-2 border-yellow-600/50 bg-gradient-to-br from-yellow-900/20 to-transparent cursor-pointer flex justify-between items-center">
                        <h2 className="text-xl font-bold text-yellow-300">Core y Estabilidad</h2>
                        <ChevronRightIcon className="text-yellow-400" />
                    </div>
                    
                    {specialCategories.length > 0 && (
                        <div>
                            <h2 className="text-2xl font-bold pt-4 mb-4">Grupos Funcionales</h2>
                            <div className="grid grid-cols-2 gap-4">
                                {specialCategories.map(categoryName => (
                                    <Card key={categoryName} onClick={() => navigateTo('chain-detail', { chainId: categoryName })} className="cursor-pointer text-center !py-6">
                    ...
                    <div>
                        ...
                        <div className="space-y-3">
                            {bodyPartCategories.map(categoryName => (
                                <div key={categoryName} onClick={() => navigateTo('muscle-category', { categoryName })} className="glass-card p-4 flex justify-between items-center cursor-pointer list-none hover:border-primary-color">
                    ...
                    {hallOfFameExercises.length > 0 && (
                        <Card>
                            <div className="flex justify-between items-center mb-3">
                                ...
                                <Button onClick={() => navigateTo('hall-of-fame')} variant="secondary" className="!text-xs !py-1">Ver todo</Button>
                            </div>
                            <div className="relative">
                                <div className="flex overflow-x-auto snap-x snap-mandatory space-x-3 pb-3 -mx-4 px-4 hide-scrollbar">
                                    {hallOfFameExercises.map(ex => (
                                        <div key={ex.id} className="snap-center flex-shrink-0 w-3/5 sm:w-2/5">
                                            <div onClick={() => navigateTo('exercise-detail', { exerciseId: ex.id })} className="h-full bg-slate-900 rounded-lg p-4 flex flex-col justify-between cursor-pointer border-2 border-yellow-900/80 hover:border-yellow-400/50 transition-colors">
                                                <h3 className="font-bold text-white">{ex.name}</h3>
                                                <p className="text-xs text-slate-400">{ex.involvedMuscles?.find(m => m.role === 'primary')?.muscle}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <details key={playlist.id} className="glass-card !p-0">
                        <summary className="p-4 cursor-pointer list-none flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-sm text-slate-400">{playlist.exerciseIds.length} ejercicios</p>
                            </div>
                            <div className="flex items-center gap-2">
                                <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleDeletePlaylist(playlist.id); }} className="p-2 text-slate-500 hover:text-red-400"><TrashIcon size={16}/></button>
                                <ChevronRightIcon className="details-arrow transition-transform" />
                            </div>
                        </summary>
                        <div className="border-t border-slate-700/50 p-2 space-y-1">
                            ...
                            }) : <p className="text-sm text-slate-500 text-center p-4">Esta lista está vacía.</p>}
                        </div>
                    </details>
                ))
            ) : (
                !isCreatingNew && <p className="text-center text-slate-500 pt-8">Aún no has creado ninguna lista.</p>
            )}
        </div>
    );
        ...
        <div className="animate-fade-in">
            <div className="sticky top-0 z-20 bg-[var(--header-bg-color-fallback)] backdrop-blur-[var(--header-bg-blur)] border-b border-[var(--header-border-color-fallback)] -mx-4 px-4 py-3 space-y-3">
                 <div className="flex bg-panel-color-light p-1 rounded-full">
                    ...
                    <button onClick={() => setActiveTab('lists')} className={`w-full py-1.5 rounded-full text-sm font-semibold flex items-center justify-center gap-2 ${activeTab === 'lists' ? 'bg-primary-color text-white' : 'text-slate-300'}`}><ClipboardListIcon size={16}/> Mis Listas</button>
                </div>
