// components/WorkoutVolumeAnalysis.tsx
import React, { useState, useEffect } from 'react';
import { Program, WorkoutLog, Settings, DetailedMuscleVolumeAnalysis, Session } from '../types';
import { calculateDetailedVolumeForAllPrograms, calculateSessionVolume } from '../services/analysisService';
...
export const WorkoutVolumeAnalysis: React.FC<WorkoutVolumeAnalysisProps> = ({ program, session, history, isOnline, settings, analysisData, title }) => {
    const { exerciseList, muscleGroupData, muscleHierarchy } = useAppContext();
    const [analysis, setAnalysis] = useState<DetailedMuscleVolumeAnalysis[] | null>(analysisData || null);
    const [isLoading, setIsLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);
    const [expandedMuscle, setExpandedMuscle] = useState<string | null>(null);
    
    useEffect(() => {
        // If analysisData is passed, use it directly.
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (program) {
                result = await calculateDetailedVolumeForAllPrograms([program], settings, exerciseList, muscleHierarchy);
            } else if (session) {
                result = calculateSessionVolume(session, exerciseList, muscleHierarchy);
            } else {
                throw new Error("Se debe proporcionar un programa o una sesión para el análisis.");
            }
            setAnalysis(result);
        } catch (err: any) {
        ...
        <Card>
            <div className="flex justify-between items-start mb-4">
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                    <BarChartIcon/> {title || 'Análisis de Volumen'}
                </h3>
                 {analysis && (
            ...
            {!isLoading && analysis && (
                    ...
                    {expandedMuscle && (() => {
                        ...
                        return (
                            <div className="mt-4 pt-4 border-t border-slate-700 animate-fade-in">
                                <div>
                                    <h4 className="text-lg font-bold text-white mb-3">Detalle de: {item.muscleGroup}</h4>
             ...
             {!isLoading && !analysis && !error && (
                 <div className="text-center py-4 space-y-3">
                    <p className="text-sm text-slate-400">Analiza el volumen de entrenamiento de esta selección, desglosado por grupo muscular.</p>
                    <Button onClick={handleGenerate} isLoading={isLoading} disabled={!isOnline}>
                        <SparklesIcon size={16}/> Analizar Volumen
                    </Button>
                </div>
             )}
        </Card>
