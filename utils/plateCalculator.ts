...
export interface PlateCombination { platesPerSide: { plate: number, count: number }[]; remainder: number; }

export const calculatePlates = (totalWeight: number, settings: Settings): PlateCombination => {
    const { weightUnit, barbellWeight } = settings;
    const availablePlates = weightUnit === 'kg' ? KG_PLATES : LBS_PLATES;
    
    if (totalWeight <= barbellWeight) return { platesPerSide: [], remainder: 0 };
    ...
    return { platesPerSide, remainder: Math.round(weightPerSide * 2 * 100) / 100 };
};